...
(defcustom  mf--header-file-extension "h"
  "*The extension to use for c++ header files.  Don't include the dot."
  :type 'string
  :group 'member-function)

(defcustom mf--source-file-extension '("cpp" "cc" "cxx" "C")
  "*A list of  extensions to use for c++ source files.
Put your preferred extension at the beginning of this list, and don't include the dot."
  :type 'list
  :group 'member-function)

(defvar mf--is-decl nil "Processing declaration or not.")
...
(defconst c++-keywords
  (sort
   (list "and" "bool" "compl" "do" "export" "goto" "namespace" "or_eq" "return"
         ...
         "unsigned" "while" "char\\*") (lambda (a b) (> (length a) (length b)))))
...
(defun mf--strip-namespace ()
  ...
  (interactive)
  (let ((r-match-namespace (rx (* blank) "namespace" (+ blank) (group (+? nonl))
                               (* blank) (* "
") (* blank) "{" ))
...
(defun mf--skip-comments ()
  ...
  (catch 'p-found
    (while (and (not (= (point) (point-max)))
      ...
      (if mf--is-decl
          (progn
            (if (looking-at (rx (* blank) (or "DISABLE_COPY(" "inline")))
                (end-of-line))
            (if (looking-at (rx (* blank)
                                (group (or "override" "final") (* blank)) ";"))
                (goto-char (match-end 1)))))


      ;; Copy constructor.
      (if (looking-at (rx (* blank) (group (+ (or alnum "_"))) (* blank)
                          "(" (* blank) "const" (+ blank) (backref 1) (* blank) "&" (* blank)
                          ");"))
          (end-of-line))
      (if (looking-at (rx (* blank) (group (+ (or alnum "_"))) (* blank) "&"
                          (* blank) (* "\n") (* blank)
      ...
      (if (looking-at (rx "/" (* space) (not (any "/"))))
          (throw 'p-found 1)))))

(defun mf--skip-headers ()
  "Skip over headers"
  (while (and (not (= (point) (point-max)))
              (not (looking-at-p "[^ \t\n/]")))
    (while (looking-at-p "[ \t\n]")       ; skip over whitespace.
      (forward-char))
    (if (looking-at-p "//")               ; skip over C++ style comments.
        (end-of-line))
    (if (looking-at-p "/\\*")             ; skip over C style comments.
        (re-search-forward "\\*/"))
    (if (looking-at-p (rx "#include" (+ blank) (or "<" "\"")))
        (end-of-line)))
    ...
    (dolist (c member-fn (reverse result))
      (cond ((eq (car c) 'parens)       ; arguments found...
                   ...
                   ((and (= (length (second result)) 2) ; handle dtor
                    ...
                    (let ((id (pop result)))
                      ...
                      (when has-template-spec
                        (mapc (lambda (arg) (push arg result))
                              (cdr template-spec)))
            ...
            ((and (eq (car c) 'keyword)
             ...
             (when has-template-spec
               (map (lambda (arg) (push arg result))
                    (cdr template-spec)))
             (push (list 'scope-resolution "::") result)
             (push c result))
            (t (push c result))))))
...
(defun mf--remove-blocks (fn-defs-list)
  ...
  (mapcar (lambda (def)
            (cl-remove-if (lambda (el) (eq (car el) 'block)) def))
          fn-defs-list))
          ...
          (let ((final
                 (dolist (el decl result-string)
                   (cond ((eq (car el) 'parens)
                         ...
                         (t (setq result-string (concat result-string spacer (cadr el)))
                            (setq spacer " "))))))
            (setq final (replace-regexp-in-string (rx (+? blank) (group (or "*" "&"))) "\\1" final))
...
(defun mf--format-undefined-decls-from-files (header-file dot-c-file)
  "Format all of the found undefined member function declarations for insertion into
the .C file."
...
(defun mf--infer-c-filename (header-filename)
  ...
  (if (string-match
       ...
       header-filename)
      (aif (projectile-get-other-files
            (buffer-file-name)
            (projectile-current-project-files))
          (format "%s/%s" (projectile-project-root) (car it))
        (aif (catch 'p-found
               (dolist (ext mf--source-file-extension)
                 (let ((tmp-sourcefile (concat (substring header-filename 0 -1) ext)))
                   (if (file-exists-p tmp-sourcefile)
                       (throw 'p-found tmp-sourcefile)))))
            it
          (concat (substring header-filename 0 -1) (car mf--source-file-extension))))
    header-filename))
...
(defun mf--make-temp-impl-buff (cfile)
...
temp buffer when he is done with it."
  (let ((tmp-buff (get-buffer-create (make-temp-name cfile)))
        (contents "")
        (cbuf (aif (get-buffer cfile) it
                (if (file-exists-p cfile) (find-file-noselect cfile))))
        )
...
(defconst yc/editor-mode-line "/*
 * Editor modelines
 *
" "A symbol of editor mode line, added by Yangyingchao@gmail.com")

;;;XXX: skip override, final....
;;;###autoload
...
implementation file."
  ...
  (let ((temp-file (mf--make-temp-impl-buff c-file))
        (h-buf  (find-file-noselect header))
        (c-buf (find-file-noselect c-file))
        src-modified namespaces tip)

    (defun mf--expand-internal ()
      "Internal function."
      (with-current-buffer c-buf
        (save-excursion
          (goto-char (point-min))
          (when (and (not (search-forward-regexp
                           (format "#include[[:blank:]]+[\"<].*?%s[\">]" header)
                           nil t))
                     (not (string= header c-file)))
            (goto-char (point-min))
            (mf--skip-headers)
            (insert (format "#include \"%s\"\n" header))
            (setq src-modified (point)))))

      ;; note: Optimized by evacuating function definition blocks in c-file.
      (with-temp-buffer
        (insert-buffer-substring (get-buffer header))
        (goto-char (point-min))
        (setq namespaces (mf--strip-namespace))
        (setq mf--saved-string
              (replace-regexp-in-string
               (rx (group alnum) (* blank)"<" (+ blank) (group alnum)) "\\1<\\2"
               (replace-regexp-in-string
                (rx (group (or alnum "*")) (+ blank) ">"  ) "\\1>"
                (replace-regexp-in-string
                 (rx (+ blank) "::" (+ blank)) "::"
                 (mf--format-undefined-decls-from-files nil temp-file))))))

      (when (and mf--saved-string
                 (> (length mf--saved-string) 0))
        (with-current-buffer c-buf
          (goto-char (point-max))
          (beginning-of-line)
          (while (or (= (line-beginning-position) (line-end-position)) ;; Empty line.
                     (string-match
                      (rx line-start (* blank) "/" (or "/" "*"))
          ...
          (unless (fboundp 'emr-cc-format-region)
            (load "emr-c.el"))
          (emr-cc-format-region src-modified (point-max)))))
    ...
    (if debug-on-error
      ...
      (condition-case err
        ...
        (error (setq tip (error-message-string err)))))

    ;; Clean up files.
    (kill-buffer temp-file)

    (if src-modified
        (progn (set-window-buffer nil c-buf)
               (message "Buffer %s modified..." c-file))
      (if tip
          (message "Error: %s, nothing was changed..." tip)
