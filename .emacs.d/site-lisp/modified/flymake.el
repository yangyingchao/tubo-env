...
;;
;; XXX: Modified by yangyingchao@gmail.com
;;    1. Integrated functions from the Internet:
;;       a) For C/CPP files without Makefile: calling gcc/g++.
;;       b) For Python scripts, calling pyflakes.
;;       c) Functions ans hot keys to focus next/previous error.
;;    2. Automatically modify Makefile if needed.
;;       a) If Makefile exists, but it does not have a rule for
;;          "check-syntax", add this rule.
;;       b) If it has this rule, do nothing.
;;    3. Do not report error if no errors or warnings was given by compiler,
;;       even if the exist code is not 0.
;;
;;; Bugs/todo:
...
(defun flymake-get-cleanup-function (file-name)
  ...
  (flymake-log 3 "YC: flymake-get-cleanup-function")
...
It's flymake process filter."
  (flymake-log 3 "yc:flymake-fprocess-filter called")
...
(defun flymake-process-sentinel (process event)
  ...
  (when (memq (process-status process) '(signal exit))
    (let* ((exit-status       (process-exit-status process))
      ...
      (condition-case err
          (progn
            (flymake-log 3 "YC: cleaning up using %s" cleanup-f)
            (when (buffer-live-p source-buffer)
              (with-current-buffer source-buffer
                (funcall cleanup-f)))
...
(defun flymake-post-syntax-check (exit-status command)
  (flymake-log 3 (format
                  "YC:  flymake-post-syntax-check called, exit-status: %d"
                  exit-status))
  ...
  (let (err-count warn-count)
    ...
    (if (and (equal 0 err-count) (equal 0 warn-count))
        (if (equal 0 exit-status)
          ...
          (progn
            (flymake-log 3 "YC: no errs or wrns, but exit code is not 0")
      ...
      (progn
        (flymake-log 2 "YC:  debug")
        (flymake-report-status (format "%d/%d" err-count warn-count) "")))))

(defun flymake-parse-output-and-residual (output)
  "Split OUTPUT into lines, merge in residual if necessary."
  (flymake-log 3 "YC:  flymake-parse-output-and-residual called.")
  (let* ((buffer-residual     flymake-output-residual)
         (total-output        (if buffer-residual
    ...
    (flymake-log 3 "YC: Leave")))

(defun flymake-parse-residual ()
  "Parse residual if it's non empty."
  (flymake-log 3 "YC: flymake-parse-residual called")
  ...
  (flymake-log 3 "YC: flymake-parse-residual leave"))
...
(defun flymake-parse-err-lines (err-info-list lines)
  ...
  (flymake-log 3 (format "YC: lines: %s" lines))
...
(defun yc/prepare-makefile (path cc)
  "Prepare makefile for flyspell."
  (message (format "(prepare-makefile): Called with: %s" path))
  (save-excursion
    (if (file-exists-p path)
        (let ((cbuffer (get-buffer-create "* MK_P *")))
          (set-buffer cbuffer)
          (insert-file-contents path)
          (if (search-forward "check-syntax:" (point-max) t)
              t
            (progn
              (goto-char (point-max))
              (insert (format "\ncheck-syntax:
  ...
  (let ((cc (if (string= (file-name-extension source) "c") "gcc" "g++")))
    (list cc
          (list "-Wall"
                ...
                "-I../../public"
                source))))
(defun flymake-init-find-makfile-dir (source-file-name cc)
    ...
    (catch 'found
      (dolist (makefile flymake-makefile-filenames)
        (let ((found-dir (flymake-find-buildfile makefile source-dir)))
          (when found-dir
            (yc/prepare-makefile (format "%s%s" found-dir makefile) cc)
  ...
  (let* ((args nil)
         ...
         (buildfile-dir
          (and (executable-find "make")
               (flymake-init-find-makfile-dir source-file-name cc)))
         )
...
(define-key flymake-mode-map (kbd "M-n") 'flymake-goto-next-error-disp)
(define-key flymake-mode-map (kbd "M-p") 'flymake-goto-prev-error-disp)
