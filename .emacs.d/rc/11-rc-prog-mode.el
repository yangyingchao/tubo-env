...
(defun yc/flymake-save-buffer-in-file (file-name bname)
  (flymake-log 3 (format "YC-%s:%s" file-name bname))
  (make-directory (file-name-directory file-name) 1)
  (set-buffer (get-buffer (file-name-nondirectory bname)))
  (write-region nil nil file-name nil)
  (flymake-log 3 "yc: saved buffer %s in file %s" (buffer-name) file-name))


(defun yc/flymake-init-create-temp-buffer-copy (create-temp-f bname)
  "Make a temporary copy of the current buffer, save its name in buffer data
and return the name."
  (message (format "YC:CCCCCCCC: %s" bname))
  (let  ((temp-source-file-name
           (funcall create-temp-f bname "flymake")))

    (yc/flymake-save-buffer-in-file temp-source-file-name bname)
...
Use CREATE-TEMP-F for creating temp copy."
  (let* ((args nil)
         ...
         (cc
          (if (string= (file-name-extension source-file-name) "c")
              "gcc"
            "g++")))
    (flymake-log 3 (format "file: %s" source-file-name))
    (if (or buildfile-dir
            (executable-find cc))
        (let* ((temp-source-file-name
                (ignore-errors
                  (yc/flymake-init-create-temp-buffer-copy create-temp-f source-file-name))))
          (if temp-source-file-name
              (progn
                (flymake-log 3 (format "YC- %s, builddir: %s, source-dir: %s"
                                       temp-source-file-name
                                       buildfile-dir
                                       source-dir))
                (setq args
