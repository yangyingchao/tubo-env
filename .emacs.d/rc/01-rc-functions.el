...
 ;; Function used to add fields of struct into a file generated by DIA.

(defvar attr_str nil "Attribute String")

(setq attr_str "<dia:composite type=\"umlattribute\">
...
(defun yyc/datastruct-to-dia (start end)
  ...
  (let* ((tmp_str "")
    ...
    (defun iter (pos)
      (if (string-match r_str var-defination pos)
          (progn
            (message "Called!")
            (setq var-type
            ...
            (setq tmp_str (concat tmp_str
                                  (format attr_str var-type var-name)))
            (iter (match-end 0)))
        nil))
    (save-excursion
      (iter 0)
      (set-buffer (get-buffer-create "tmp.aaa"))
      (nxml-mode)
      (setq pos (point-max))
      (insert tmp_str)
      (indent-region pos (point-max)))
    (if (one-window-p)
        (split-window-vertically))
    (switch-to-buffer-other-window "tmp.aaa")
    (goto-char (point-min))
    (kill-line)
    )
  )
