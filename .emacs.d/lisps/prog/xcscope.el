...
(defun cscope-update-db()
  (interactive)
  (if (not (string= cscope-initial-directory nil))
    (let ( (outbuf (get-buffer-create cscope-output-buffer-name))
           (old-buffer (current-buffer)) )
      (if cscope-process
          (error "A cscope search is still in progress."))
      (message  "Update cscope database")
      (start-process "UpdateDB" outbuf "python"
                     "d:/gnu/bin/cscope_update_db.py"
                     cscope-initial-directory)
    (message "Finished update database."))
  (message "cscope-initial-directory is nil, set it first!")))
