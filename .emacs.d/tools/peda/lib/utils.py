...
def debug_msg(text, prefix="Debug"):
    ...
    if not config.Option.get("debug") == "on":
        return
...
def gen_strings(string):
    """
    Generate list of substrings for input string.

    For example, gen_strings('ABCD') will result list ['ABCD', 'ABC','AB', 'A' ]
    """
    ...
    ret = string
    while len(ret) > 0:
        yield string[0:len(ret)]
        ret = string[0:len(ret)-1]
    pass
